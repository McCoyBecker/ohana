...
class Parser(object):
    ...
    def _parse_identifier_expr(self):
        ...
        if not self._cur_tok_is_operator('('):
            return ast.VariableExprAST(id_name)
        ...
        return ast.CallExprAST(id_name, args)

    # numberexpr ::= number
    def _parse_number_expr(self):
        result = ast.NumberExprAST(self.cur_tok.value)
    ...
    def _parse_binop_rhs(self, expr_prec, lhs):
        ...
        while True:
            ...
            # Merge lhs/rhs
            lhs = ast.BinOpExprAST(op, lhs, rhs)
    ...
    def _parse_prototype(self):
        ...
        while self.cur_tok.kind == TokenKind.IDENTIFIER:
            ...
            self._get_next_token()
        self._match(TokenKind.OPERATOR, ')')
        return ast.PrototypeAST(name, argnames)
    ...
    def _parse_toplevel_expression(self):
        expr = self._parse_expression()
        # Anonymous function
        return ast.FunctionAST.create_anonymous(expr)
